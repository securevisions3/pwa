...
/* ─── Auth: reset completo — borra todo y deja la UI lista para reconectar ─── */
function resetAuth(){
  // 1. Borrar todos los tokens (localStorage)
  clearTokens();
  // 2. Borrar verifier PKCE huérfano (sessionStorage)
  sessionStorage.removeItem('sv_pkce_verifier');
  // 3. Resetear estado de la app
  state.gmailReady    = false;
  state.selectedEmail = '';
  state.allEvents     = [];
  state.pageToken     = null;
  // 4. Resetear UI del modal
  els.gmailAccount.textContent        = '';
  ...
  renderHistory();
  toast('Caché limpiado. Pulsa Conectar para iniciar sesión.');
}
...
async function initAuth(){
  ...
  if(location.search.includes('code=')){
    ...
    if(!ok){ els.gmailStatus.textContent='Error en login'; els.btnGmailConnect.style.display='inline-flex'; return; }
  }
...
function bind(){
  ...
  els.btnClearAuth.onclick    = resetAuth;
